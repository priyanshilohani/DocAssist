...
export default function TextEditor() {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!token) {
      alert("You need to log in before uploading files.");
      return;
    }
  ...
  const handleGetSuggestions = async () => {
    const token = localStorage.getItem("jwtToken"); // Fetch token from localStorage
    if (!token) {
      alert("You need to log in before fetching suggestions.");
      return;
    }
    ...
    try {
      const response = await fetch("http://127.0.0.1:5000/get-suggestions", {
        ...
        body: JSON.stringify({ user_input: userInput, document_id: uploadedFile?.name.split(".")[0] }),
      });

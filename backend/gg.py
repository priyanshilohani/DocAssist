...
# Set up logging
logging.basicConfig(level=logging.DEBUG)  # Log everything at the DEBUG level and above
...
def process_pdf(file):
    try:
        with pdfplumber.open(file) as pdf:
            content = "".join(page.extract_text() for page in pdf.pages)
            if not content:
                logger.warning("No text found in the PDF.")  # If no text is extracted
            else:
                logger.info(f"Extracted content: {content[:500]}...")  # Show first 500 characters for debugging
    except Exception as e:
...
def decode_jwt(token):
    try:
        decoded_token = jwt.decode(token, SECRET_KEY, algorithms=["HS256"])
        logger.debug(f"Decoded JWT Token: {decoded_token}")  # Debugging token
        return decoded_token
    except jwt.ExpiredSignatureError:
        logger.error("JWT Token expired")  # Token expired
        return None
    except jwt.InvalidTokenError:
        logger.error("Invalid JWT Token")  # Invalid token
        return None
...
def process_document():
    try:
        ...
        # Log the received files
        for file in uploaded_files:
            logger.info(f"Received file: {file.filename}")  # Log the filename to verify it's being received
        ...
        try:
            documents_collection.insert_one(document_data)
            logger.info("Document saved to MongoDB successfully!")
            return jsonify({"message": "Files processed successfully!", "chunks": all_chunks})

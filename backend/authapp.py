...
# # Secret key for encoding JWT tokens (Store securely, e.g., in environment variables)
...
#         jwt.decode(token, SECRET_KEY, algorithms=["HS256"])  # Decode to check validity
...
# # Signout Route (Different from logout, in case of multiple sessions)
...
#         return jsonify({"message": f"User {decoded_token['username']} signed out successfully!"}), 200
#     except jwt.ExpiredSignatureError:
#         return jsonify({"error": "Token has already expired!"}), 401
#     except jwt.InvalidTokenError:
#         return jsonify({"error": "Invalid token!"}), 401
    ...
    try:
        ...
        if bcrypt.checkpw(password.encode('utf-8'), user["password"].encode('utf-8')):
            ...
            access_token = jwt.encode({
                "user_id": user["email"],
                "username": user["username"],
                "exp": datetime.now(timezone.utc) + timedelta(hours=2)  # 🔥 Now valid for 2 hours
            }, SECRET_KEY, algorithm="HS256")

            refresh_token = jwt.encode({
                "user_id": user["email"],
                "exp": datetime.now(timezone.utc) + timedelta(days=7)  # 🔥 Refresh token lasts 7 days
            }, SECRET_KEY, algorithm="HS256")
